...
const showDeleteResumeConfirmationModal = ref(false)
const { data: resume, error } = await useAPI<ICreateResumeResponse>(`/resumes`)

async function handleDeleteResume() {
	const { error } = await useAPI(`/resumes/${resume?.id}`, {
		method: "DELETE",
	})
...
<template>
	<div class="container flex-1 px-6 py-6 flex flex-col md:max-w-3xl gap-y-6 mx-auto">
		<section class="flex flex-col gap-y-4">
			...
			<div class="flex flex-col gap-y-2 empty:hidden">
				<JobInfoIcon :icon="RiPhoneLine">{{ auth.user?.phone }}</JobInfoIcon>
				<JobInfoIcon :icon="RiMailLine">{{ auth.user?.email }}</JobInfoIcon>
			</div>
		</section>

		<main class="flex flex-col items-center gap-y-4 mt-16" v-if="!resume">
			<h1 class="text-lg text-center text-slate-900 font-medium font-lexend">
				Você não possui um currículo cadastrado
			</h1>
			<LinkButton href="/candidate/resume" class="md:w-auto">
				<RiPencilLine class="fill-current h-6 w-6" />
				Cadastrar currículo
			</LinkButton>
		</main>

		<main class="flex flex-col items-center gap-y-6" v-else>
		...
		<footer class="flex gap-x-2" v-if="resume">
			<Button
				class="!button-primary-outline md:w-auto"
				@click="showDeleteResumeConfirmationModal = true"
			>
				<RiDeleteBinLine class="fill-current h-6 w-6" />
				Excluir
			</Button>
	...
	<ModalContainer
		:show="showDeleteResumeConfirmationModal"
		@close="showDeleteResumeConfirmationModal = false"
	>
		<span
			...
			>!</span
		>
		<p class="text-xl text-slate-900 font-semibold">Deseja excluir o currículo?</p>
		<p class="text-neutral-500">
			Você está prestes a excluir o seu currículo. Essa ação é irreversível. Por favor, confirme sua
			decisão.
		</p>
		<div class="flex w-full flex-col gap-y-2">
			<Button class="!button-primary-outline" @click="showDeleteResumeConfirmationModal = false"
				>Cancelar</Button
			>
			<Button class="!bg-red-500 hover:!bg-red-600" @click="handleDeleteResume">Excluir</Button>
		</div>
	</ModalContainer>
</template>
